...
Template.message.helpers({
	...
	date() {
		//2017-10-17,Edison changed: 如果日期是今天，则返回小时和分钟，否则返回日期加小时和分钟
		if(this.ts.getMinutes() < 10){
			var minute = "0"+this.ts.getMinutes();
		}else{
			var minute = this.ts.getMinutes();
		}
		if(this.ts.getHours() < 10){
			var hour = "0"+this.ts.getHours();
		}else{
			var hour = this.ts.getHours();
		}
		var hour_minute = hour +":"+minute;
		if(this.ts.toDateString() !== new Date().toDateString()){
			var date = moment(this.ts).format(RocketChat.settings.get('Message_DateFormat'));
	...
	return this._domrange.onAttached(function(domRange) {
		...
		if (previousNode == null) {
			...
			if (previousMessageDate.toDateString() !== currentMessageDate.toDateString()) {
				...
				//Edison changed: 如果两次发言时间相隔30分钟，则显示出来时分
				var time_difference = parseInt(currentDataset.timestamp) - parseInt(previousDataset.timestamp);
				if(time_difference > 60 * 30 * 1000){
					$currentNode.addClass('new-day');
				}else{
