...
Template.createChannel.events({
	...
	'submit .create-channel__content'(e, instance) {
		...
		// wgb 2017-10-16 17:16:07
		var fname = $(e.target).find('[name=fname]').val();
		if(fname == '') { 
			fname = name; 
		}
		...
		Meteor.call(isPrivate ? 'createPrivateGroup' : 'createChannel', name, fname, instance.selectedUsers.get().map(user => user.username), readOnly, function(err, result) {
			if (err) {
