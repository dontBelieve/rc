...
RocketChat.API.v1.addRoute('channels.create', { authRequired: true }, {
	post() {
		...
		// wgb 2017-10-31 17:30:40
		let id;
		Meteor.runAsUser(this.userId, () => {
			id = Meteor.call('createChannel', this.bodyParams.name, this.bodyParams.fname ? this.bodyParams.fname : this.bodyParams.name, this.bodyParams.members ? this.bodyParams.members : [], readOnly, this.bodyParams.customFields);
		});
...
// wgb 2017-11-2 09:30:37
RocketChat.API.v1.addRoute('channels.renamef', { authRequired: true }, {
	post() {
		const findResult = findChannelByIdOrName({ params: { roomId: this.bodyParams.roomId} });

		Meteor.runAsUser(this.userId, () => {
			Meteor.call('saveRoomSettings', findResult._id, 'roomFname', this.bodyParams.fname);
		});
