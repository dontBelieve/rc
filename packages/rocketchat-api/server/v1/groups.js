...
RocketChat.API.v1.addRoute('groups.create', { authRequired: true }, {
	post() {
		...
		// 2017-11-6 10:56:29 wgb
		let id;
		Meteor.runAsUser(this.userId, () => {
			id = Meteor.call('createPrivateGroup', this.bodyParams.name, this.bodyParams.fname ? this.bodyParams.fname : this.bodyParams.name, this.bodyParams.members ? this.bodyParams.members : [], readOnly, this.bodyParams.customFields);
		});
...
// wgb 2017-11-6 10:57:58
RocketChat.API.v1.addRoute('groups.renamef', { authRequired: true }, {
	post() {
		const findResult = findPrivateGroupByIdOrName({ params: { roomId: this.bodyParams.roomId}, userId: this.userId });

		Meteor.runAsUser(this.userId, () => {
			Meteor.call('saveRoomSettings', findResult.rid, 'roomFname', this.bodyParams.fname);
		});
