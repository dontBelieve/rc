...
Template.channelSettings.onCreated(function() {
	...
	this.settings = {
		...
		fname: {
			...
			save(value, room) {
				return Meteor.call('saveRoomSettings', room._id, 'roomFname', value, function(err) {
					if (err) {
						return handleError(err);
					}
					toastr.success(TAPi18n.__('Room_name_changed_successfully'));
					return RocketChat.callbacks.run('roomFnameChanged', room);
				});
			}
		},
