Meteor.methods({
	saveRoomSettings(rid, setting, value) {
		...
		// wgb 2017-10-17 13:50:46	roomFname
		if (!['roomName', 'roomFname', 'roomTopic', 'roomAnnouncement', 'roomDescription', 'roomType', 'readOnly', 'reactWhenReadOnly', 'systemMessages', 'default', 'joinCode'].some((s) => s === setting)) {
		...
		if (room != null) {
			...
			switch (setting) {
				case 'roomName':
					...
					break;
				// wgb 2017-10-17 13:51:47
				case 'roomFname':
					if (value !== room.fname) {
						RocketChat.saveRoomFname(rid, value, Meteor.user());
					}
					break;
